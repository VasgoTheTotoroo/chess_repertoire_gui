...
# C:\Users\vassia\Desktop\echec\transposition_finder\deprecated\transposition_finder.py
# C:\Users\vassia\Desktop\echec\transposition_finder\deprecated\split_pgn.py "game1.txt"
# C:\Users\vassia\Desktop\echec\transposition_finder\deprecated\deviation_finder.py

class Move():
    def __init__(self, name, fen=None, comments=None, parent=None):
        self.name = name
        self.parent = parent
        self.fen = fen
...
for d in D:
    ...
    for idx in d:
        ...
        if len(moves[idx].children) > 0:
            if moves[idx].children[0].comments:
                if moves[idx].children[0].comments.find("Transposition") > -1:
                    nb_comments+=1
...
def french_chess(string):
    last = "Transposition "+string.replace("N", "C").replace("B", "F").replace("R", "T").replace("Q", "D").replace("K", "R")
    last = re.sub(r"\.\ ", ".", last)
    last = re.sub(r"\d+\.\.\.", "", last)
    return last

for i, dup in enumerate(reversed(true_duplicates)):
    strings = []
    for idx in dup:
        strings.append(french_chess(moves[idx].str_to_root()))
